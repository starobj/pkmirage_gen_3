...
#define I_SITRUS_BERRY_HEAL             GEN_4       // In Gen4+, Sitrus Berry was changed from healing 30 HP to healing 25% of Max HP.
...
#define I_GRISEOUS_ORB_FORM_CHANGE      GEN_4       // In Gen9+, the Griseous Orb no longer changes Giratina's form when held.
#define I_GEM_BOOST_POWER               GEN_5       // In Gen6+, the Gem boost power was reduced from 50% to 30%.
...
#define I_TYPE_BOOST_POWER              GEN_4       // In Gen4+, type-enhancing held items (such as Charcoal) had their multiplier increased from 1.1x to 1.2x. Sea Incense was changed from 1.05x to 1.2x.
...
#define I_BERRY_PRICE                   GEN_6       // Since Berries have become unplantable (Gen8+), their price has gone up.
#define I_POWER_ITEM_BOOST              GEN_7       // In Gen7+, Power Items grant 8 EVs instead of 4 EVs.
#define I_PREMIER_BALL_BONUS            GEN_8       // In LGPE onwards (Gen8+ here), you are given a Premier Ball for every 10 Poké Balls of any type and in the same purchase. Previously, it only applied to regular Poké Balls and only 1 could be obtained per purchase.
...
#define I_REPEL_INCLUDE_FAINTED         GEN_4       // In Gen1 and Gen6+, Repels always use the level of the first member of the party to check which wild Pokémon to prevent encounters with, even if that member is fainted. In Gen2-5, it only uses the level of the first non-fainted Pokémon.
#define I_LAX_INCENSE_BOOST             GEN_4       // In Gen4+, Lax Incense increases evasion by 10% instead of 5%.
...
// Vs. Seeker
#define I_VS_SEEKER_CHARGING        0 // FLAG_VS_SEEKER_CHARGING     // If this flag is assigned, the Vs Seeker functionality will be enabled. When the player has the Vs. Seeker, Match Call rematch functions will stop working. Documentation for the Vs. Seeker can be found in docs/tutorials/vs_seeker.md.
